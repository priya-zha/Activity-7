...
var version = exports.version = "6.7.0";
...
var _d = require("d3");
var scatterPlot = exports.scatterPlot = function scatterPlot() {
  var width;
  var height;
  var data;
  var xValue;
  ...
  var my = function my(selection) {
    var x = (0, _d.scaleLinear)().domain((0, _d.extent)(data, xValue)).range([margin.left, width - margin.right]);
    var y = (0, _d.scaleLinear)().domain((0, _d.extent)(data, yValue)).range([height - margin.bottom, margin.top]);
    var marks = data.map(function (d) {
      return {
        x: x(xValue(d)),
        y: y(yValue(d))
      };
    });
    var circles = selection.selectAll('circle').data(marks).join('circle').attr('cx', function (d) {
      return d.x;
    }).attr('cy', function (d) {
      return d.y;
    }).attr('r', radius);
    selection.selectAll('.y-axis').data([null]).join('g').attr('class', 'y-axis').attr('transform', "translate(".concat(margin.left, ",0)")).call((0, _d.axisLeft)(y));
    selection.selectAll('.x-axis').data([null]).join('g').attr('class', 'x-axis').attr('transform', "translate(0,".concat(height - margin.bottom, ")")).call((0, _d.axisBottom)(x));
...
function _asyncToGenerator(n) { return function () { var t = this, e = arguments; return new Promise(function (r, o) { var a = n.apply(t, e); function _next(n) { asyncGeneratorStep(a, r, o, _next, _throw, "next", n); } function _throw(n) { asyncGeneratorStep(a, r, o, _next, _throw, "throw", n); } _next(void 0); }); }; } //import * as d3 from 'https://cdn.skypack.dev/d3';
var csvUrl = ['https://gist.githubusercontent.com/priya-zha/3dc3aa9a47007d7611c0e32d6a93ee3d/raw/32dc00c22df5f1d665c6b63181089d5eade9765f/iris.csv' // File name
].join('');
var parseRow = function parseRow(d) {
...
var svg = (0, _d.select)('body').append('svg').attr('width', width).attr('height', height);
...
var rxValue = function rxValue(d) {
  return d.sepal_length;
};
var ryValue = function ryValue(d) {
  return d.sepal_width;
};
var speciesValue = function speciesValue(d) {
  return d.species;
};
var colorValue = function colorValue(d) {
  return d.species;
};
var colorLabel = 'Species';
var main = /*#__PURE__*/function () {
  var _ref = _asyncToGenerator(/*#__PURE__*/_regeneratorRuntime().mark(function _callee() {
    var data, x, y, marker, marks, plot, g, colorLegendG, columns, i;
    return _regeneratorRuntime().wrap(function _callee$(_context) {
      while (1) switch (_context.prev = _context.next) {
        case 0:
          _context.next = 2;
          return (0, _d.csv)(csvUrl, parseRow);
        case 2:
          data = _context.sent;
          x = xValue(function (d) {
            return d.petal_width;
          ...
          _context.t0 = (0, _scatterPlot.scatterPlot)().width(width).height(height);
          _context.next = 10;
          return (0, _d.csv)(csvUrl, parseRow);
        case 10:
          _context.t1 = _context.sent;
          plot = _context.t0.data.call(_context.t0, _context.t1).xValue(function (d) {
            return d.petal_width;
          }).yValue(function (d) {
            return d.sepal_length;
          }).margin({
          ...
          g = svg.append('g').attr('transform', "translate(".concat(100, ",", 100, ")"));
          colorLegendG = g.append('g').attr('transform', "translate(".concat(100, ", 150)")); //shape of that scattered data 
          ...
          columns = ['petal_width', 'sepal_width', 'petal_length', 'sepal_length'];
          i = 0;
          setInterval(function () {
            plot.xValue(function (d) {
              return d[columns[i % columns.length]];
            });
            svg.selectAll('text').remove();
            svg.call(plot);
            colorLegendG.append('text').attr('class', 'legend-label').attr('x', -70).attr('y', -210).style("font-style", "italic").style("stroke", "violet").style("font-weight", "bold").style("font-style", "italic").text(colorLabel);

            // display Title 
            svg.append("text").attr("x", 450).attr("y", 20).text("Iris Scatterplot").style("font-size", "40px").attr("alignment-baseline", "middle").attr('fill', 'lightgreen').style("font-style", "italic").style("text-decoration", "underline");
            ...
            svg.append("text").attr('transform', 'rotate(-90)').attr('x', 0 - height / 2).attr("y", height / 12).attr('fill', 'red').style("text-anchor", "middle").style("font-style", "italic").style("font-weight", "bold").text("Sepal Length");
            ...
            if (i % columns.length == 0) svg.append("text").attr("x", width / 2).attr("y", 480).attr('fill', 'red').style("text-anchor", "middle").style("font-style", "italic").style("font-weight", "bold").text("Petal Width");
            // plot sepal_width on x-axis
            else if (i % columns.length == 1) svg.append("text").attr("x", width / 2).attr("y", 480).attr('fill', 'red').style("text-anchor", "middle").style("font-style", "italic").style("font-weight", "bold").text("Sepal Width");
            // plot petal_length on x-axis
            else if (i % columns.length == 2) svg.append("text").attr("x", width / 2).attr("y", 520).attr('fill', 'red').style("text-anchor", "middle").style("font-weight", "bold").text("Petal Length");
            // plot sepal_length on x-axis
            else svg.append("text").attr("x", width / 2).attr("y", 540).style("text-anchor", "middle").style("font-style", "italic").style("font-weight", "bold").text("Sepal Length");
            i++;
...
main();
},{"d3":"node_modules/d3/index.js","./scatterPlot":"scatterPlot.js"}],"../../../AppData/Roaming/npm/node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
//# sourceMappingURL=/Activity%207.e31bb0bc.js.map